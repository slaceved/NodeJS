...
function querystring_stringify (obj, sep, eq, name, escape) {
  ...
  if (util.isArray(obj)) {
    var s = [];
    name = name+'[]';
    for (var i = 0, l = obj.length; i < l; i++) { //eslint-disable-line block-scoped-var
      s.push( querystring_stringify(obj[i], sep, eq, name, escape) ); //eslint-disable-line block-scoped-var
    }
    return s.join(sep);
  }

  // Check for cyclical references in nested objects
  for (var i = stack.length - 1; i >= 0; --i){ //eslint-disable-line block-scoped-var
     if (stack[i] === obj) { //eslint-disable-line block-scoped-var
  ...
  var s = []; //eslint-disable-line block-scoped-var
  var begin = name ? name + '[' : '';
  var end = name ? ']' : '';
  for (var i in obj) { //eslint-disable-line block-scoped-var
    if (obj.hasOwnProperty(i)) { //eslint-disable-line block-scoped-var
    var n = begin + i + end; //eslint-disable-line block-scoped-var
    s.push(querystring_stringify(obj[i], sep, eq, n, escape)); //eslint-disable-line block-scoped-var
    }
  }

  stack.pop();

  s = s.join(sep); //eslint-disable-line block-scoped-var
  if (!s && name){ //eslint-disable-line block-scoped-var
    return name + '=';
  }
  return s; //eslint-disable-line block-scoped-var
};
